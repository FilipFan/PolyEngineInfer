...
class MediaPipeInferenceTest {

    companion object {
        private const val TAG = "MediaPipeInferenceTest"
        private lateinit var modelFile: File
        private lateinit var mediaPipeInference: MediaPipeInference
        ...
        fun setUpClass() {
            ...
            modelFile = File(externalDir, "test_lm_new_metadata.task")
            mediaPipeInference = MediaPipeInference(instrumentationContext)
        }
    }
    ...
    fun tearDown() {
        runBlocking {
            mediaPipeInference.unload()
        }
    }

    @Test
    fun testLoadAndGenerateWithDefaultOptions() = runBlocking {
        mediaPipeInference.load(LlmModelFiles(modelPath = modelFile.path), LlmInferenceOptions())
        val prompt = "Hello"
        val generatedTokens = mediaPipeInference.generate(prompt).toList()
    ...
    fun testGenerationWithCustomMaxLength() = runBlocking {
        ...
        mediaPipeInference.load(LlmModelFiles(modelPath = modelFile.path), customOptions)
    ...
    fun testGreedyDecodingWithZeroTemperature() = runBlocking {
        ...
        mediaPipeInference.load(LlmModelFiles(modelPath = modelFile.path), greedyOptions)
        val prompt = "Hello"

        val firstResult = mediaPipeInference.generate(prompt).toList().joinToString("")
        val secondResult = mediaPipeInference.generate(prompt).toList().joinToString("")
    ...
    fun testNonDeterministicGenerationWithTemperature() = runBlocking {
        ...
        mediaPipeInference.load(LlmModelFiles(modelPath = modelFile.path), stochasticOptions)
        val prompt = "Hello"
    ...
    fun testTopKParameterProducesDeterministicResultWhenKIsOne() = runBlocking {
        ...
        mediaPipeInference.load(LlmModelFiles(modelPath = modelFile.path), topKOptions)
        val prompt = "Hello"
